...
, "cifs-utils"  -- Utilities for managing mounts of SMB/CIFS (Common Internet
...
-- Neovim Python provider dependencies for the case when they needed to be
-- installed via Pip:
...
, "netcat-openbsd"  -- OpenBSD rewrite of netcat, it includes support for IPv6,
                    -- proxies, and Unix sockets

, "socat"  -- Socat (for SOcket CAT) establishes two bidirectional byte streams
           --   and transfers data between them. Data channels may be files,
           --   pipes, devices (terminal or modem, etc.), or sockets (Unix,
           --   IPv4, IPv6, raw, UDP, TCP, SSL). It provides forking, logging
           --   and tracing, different modes for interprocess communication and
           --   many more options.

, "corkscrew"  -- A simple tool to tunnel TCP connections through an HTTP proxy
               --   supporting the CONNECT method. It reads stdin and writes to
